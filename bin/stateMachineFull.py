...
def main():
    ...
    with sm0:
        ...
        smach.StateMachine.add('SAY_INTRO',
                               ...
                               {'succeeded':'SHOW_WEBSTREAM'})
        
        smach.StateMachine.add('SHOW_WEBSTREAM',
                               smach_ros.SimpleActionState('naoqi_openWebsite_server/openWebsite', NaoQi_openWebsiteAction,
                                                           goal = NaoQi_openWebsiteGoal(url=String(data="http://149.201.37.169:8000/action_ws/src/face_learning_actions/pepper_videostream.html"))),
                               {'succeeded':'FACE_DETECTION', 'aborted':'FACE_DETECTION'})
        ...
        smach.StateMachine.add('SAY_NEW_FACE',
                               smach_ros.SimpleActionState('naoqi_animatedSay_server/animatedSay',
                                                           NaoQi_animatedSayAction,
                                                           goal = NaoQi_animatedSayGoal(animatedMessage=String(data="Ich habe ein neues Gesicht entdeckt und speichere es. Bitte trage gleich deinen Namen ein."))),
